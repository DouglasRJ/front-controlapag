...
export default function HomeScreen() {
    ...
    const handleSectionLayout = (id: string) => () => {
        const targetRef = sectionRefs.current[id];
        const scrollNode = scrollViewRef.current;
        if (!targetRef || !scrollNode) return;

        if (Platform.OS === "web") {
            // Na web, usa métodos DOM nativos
            const targetElement = targetRef as any as HTMLElement;
            const scrollElement = scrollNode as any;
            if (targetElement && scrollElement) {
                // Calcula a posição relativa ao container de scroll
                const targetRect = targetElement.getBoundingClientRect();
                const scrollRect = scrollElement.getBoundingClientRect?.() || { top: 0 };
                const scrollTop = scrollElement.scrollTop || 0;
                const relativeY = targetRect.top - scrollRect.top + scrollTop;
                sectionPositions.current[id] = relativeY;
            }
        } else {
            // No React Native, usa measureLayout
            ...
            if (nodeHandle) {
                targetRef.measureLayout(
                    ...
                    () => {
                        console.warn(`Could not measure layout for section: ${id}`);
                    }
                );
            }
        }
    ...
    const handleNavigate = (id: string) => {
        const scrollNode = scrollViewRef.current;
        const targetRef = sectionRefs.current[id];
        if (!scrollNode || !targetRef) return;

        const HEADER_OFFSET = 64;

        // Tenta usar a posição armazenada primeiro
        const storedPosition = sectionPositions.current[id];
        if (storedPosition !== undefined) {
            scrollNode.scrollTo({
                y: Math.max(0, storedPosition - HEADER_OFFSET),
                animated: true,
            });
            return;
        }

        // Fallback: mede a posição em tempo real
        if (Platform.OS === "web") {
            const targetElement = targetRef as any as HTMLElement;
            const scrollElement = scrollNode as any;
            if (targetElement && scrollElement) {
                ...
                const scrollRect = scrollElement.getBoundingClientRect?.() || { top: 0 };
                const scrollTop = scrollElement.scrollTop || 0;
                const targetY = targetRect.top - scrollRect.top + scrollTop;

                scrollNode.scrollTo({
                    y: Math.max(0, targetY - HEADER_OFFSET),
                    animated: true,
                });
            }
        } else {
        ...
        <View className="flex-1 bg-background">
            ...
            >
                ...
                <View
                    ...
                    onLayout={handleSectionLayout("funcionalidades")}
                >
                    <View
                        aria-hidden
                ...
                <View
                    ...
                    onLayout={handleSectionLayout("beneficios")}
                >
                    <View
                        aria-hidden
                ...
                <View
                    ...
                    onLayout={handleSectionLayout("como-funciona")}
                >
                    <View
                        aria-hidden
